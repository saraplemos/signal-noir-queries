...
export const MODE2_USER_PROMPT = (queries, queryFraming, publications) => `${queryFraming ? `${queryFraming}\n\n` : ""}Answer each luxury travel query below. For each answer, prioritise citing established editorial publications and media — particularly these titles if relevant: ${publications && publications.length ? publications.join(', ') : 'Condé Nast Traveller, Tatler, Vogue, Harper\'s Bazaar, Robb Report, FT How to Spend It, The Times, The Telegraph, Forbes, Elle'}.

For each query:
1. Brief answer (2-3 sentences)
2. Sources cited — prioritise the editorial publications listed above where applicable
...
Be specific with source names. Ensure you include ALL editorial sources used. Do not add any text before or after the table. Do not wrap the table in code blocks or markdown fences.
